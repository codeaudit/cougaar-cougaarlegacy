 ...
 * information from Weather Channel Web site weather pages.  Note that this 
 * is heavily adapted from example InfoAgent external query functions supplied 
 * by the CMU RETSINA support organization.
 *
 * @author Brandon L. Buteau
 * @version 1.1
 * @since 1.0
 */
public class WCNForecastWeatherQueryFn implements ExternalQueryInterface {
  ...
  // instantiate the WCNForecastWeatherQueryFn, then the BasicAgent, get
  ...
  public synchronized void setCommunicator(Communicator comm) {
    if (comm == null) {
      displayMessage("ERROR", "WCNForecastWeatherQueryFn::setCommunicator " +
  ...
  ****************************************************************/
  ...
  /** Constructor takes a QueryConfigurationObject from which it
      ...
      file by InfoAgent.
  */
  public WCNForecastWeatherQueryFn(QueryConfigurationObject queryConfigObj) {
    ...
    if (options == 3) {
      ...
      WCNForecastWeatherQueryFn.FILE_READ = false;
      WCNForecastWeatherQueryFn.FILE_WRITE = false;
    } else if (options == 1) {
      WCNForecastWeatherQueryFn.FILE_READ = false;
      WCNForecastWeatherQueryFn.FILE_WRITE = true;
    } else {
      WCNForecastWeatherQueryFn.FILE_READ = true;
      WCNForecastWeatherQueryFn.FILE_WRITE = false;
    }
  ...
  public String fetchPage(String pageURLString) {
    ...
    try {
      ...
      displayMessage("ERROR", "WCNForecastWeatherQueryFn::fetchPage " + 
  ...
  private synchronized String getRecordFields(String key, String param)     {
    ...
    try {
      ...
      displayMessage("ERROR", "WCNForecastWeatherQueryFn::getRecordFields " + 
		     "Can't load content from " + formURLString);
      return null;
    }
    ...
    while ( stillReading ) {
      try {
        ...
        displayMessage("ERROR", "WCNForecastWeatherQueryFn::getRecordFields " + 
                       "IO error getting content from " + formURLString);
        return null;
      }
      if ( ccount > 0 )
        sbuf.append(cbuf, 0, ccount);
      if ( ccount < 0 )
        stillReading = false;
    }
    //displayMessage("DEBUG", "WCNForecastWeatherQueryFn::getRecordFields " + 
    ...
    */
	...
	formURLString = " ";
      ...
      String reply = "(reply :" + CITY_STRING + " (" + key + ") " + 
	             ...
	             ":" + WEATHER_URL_STRING + " (" + formURLString + "))";
      displayMessage("DEBUG", "WCNForecastWeatherQueryFn::getRecordFields " + 
		     "Weather is: " + reply);
      return reply;
  ...
  private Parser readContent() {
    ...
    try {
      ...
      if (DEBUG) displayMessage("DEBUG", "WCNForecastWeatherQueryFn.readContent:: File " +
  ...
  private void writeContent(String content) {
    ...
    try {
      ...
      if (DEBUG) displayMessage("DEBUG", "WCNForecastWeatherQueryFn.writeContent:: File " +
  ...
  private static void setDebugFlags() {
    ...
    WCNForecastWeatherQueryFn.DEBUG = true;

    // These flags below are used to set up and turn on
    // the WCNForecastWeatherQueryFn to operate with files on disk instead
    ...
    WCNForecastWeatherQueryFn.FILE_READ = false;
    WCNForecastWeatherQueryFn.FILE_READ_PATH = "";
    WCNForecastWeatherQueryFn.FILE_READ_BASE_NAME = "weatherCNN";
    WCNForecastWeatherQueryFn.FILE_READ_EXT_NUM = 0;
    WCNForecastWeatherQueryFn.FILE_WRITE = false;
    WCNForecastWeatherQueryFn.FILE_WRITE_PATH = "";
    WCNForecastWeatherQueryFn.FILE_WRITE_BASE_NAME = "weatherCNN";
    WCNForecastWeatherQueryFn.FILE_WRITE_EXT_NUM = 0;
  }

  private static void setNoDebugFlags() {

    Parser.DEBUG = false;  // turn Parser debug off now so tasks don't timeout
    WCNForecastWeatherQueryFn.DEBUG = false;
