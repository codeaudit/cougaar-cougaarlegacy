...
public class TimeSeriesGeneratorPlugIn extends SimplePlugIn
{
  ...
  private final static long TIME_STEP = 2000;   //this is the time increment for all time series
  ...
  private int mixedPeriodCount = 0;
  private int mixedPatternSize = 7;
  private int preCursorCount = 0;
  private int mixedPatternOffset = rand.nextInt (15);
  ...
  private boolean mixedSignalStarted = false;
  ...
  public void execute ()
  {
    ...
    if (demoCommandSub != null && !demoCommandSub.isEmpty())
    {
      ...
      while (commandIter.hasNext())
      {
        ...
        else if (command.equals("mixedSignal") && mixedSignalStarted == false)
          mixedSignalStarted = true;
    ...
    if (wasAwakened())
    {
      ...
      {
        Integer leadingVal = null;
        if (crossCount >= 10 && crossCount < 15)
          leadingVal = new Integer (crossCount);
        else if (crossCount >= 25 && crossCount < 35)
          leadingVal = new Integer (crossCount);
        else
          leadingVal = new Integer (5);

        TimeSeriesValue leadTSV = new TimeSeriesValue ("LeadingTS", leadingVal, reportTime, TIME_STEP);
        publishAdd (leadTSV);


        Integer laggingVal = null;
        if (crossCount >= 15 && crossCount < 20)
          laggingVal = new Integer (crossCount + 5);
        else if (crossCount >= 30 && crossCount < 40)
          laggingVal = new Integer (crossCount + 5);
        else
          laggingVal = new Integer (5);

        TimeSeriesValue laggingTSV = new TimeSeriesValue ("LaggingTS", laggingVal, reportTime, TIME_STEP);
      ...
      if (this.movingAverageStarted)
      {
        ...
        Integer movingAvgVal = new Integer (randVal + val);

        TimeSeriesValue movingAvgTSV = new TimeSeriesValue ("MovingAvgData", movingAvgVal, reportTime, TIME_STEP);
      ...
      {
        Integer autoCorrVal = null;
        if (autoCorrCount >= 0 && autoCorrCount < 10)
        {
          autoCorrVal = new Integer (rand.nextInt (100));
        }
        else
          autoCorrVal = new Integer (50);
        ...
        TimeSeriesValue autoCorrTSV = new TimeSeriesValue
                                    ("AutoCorrelationData", autoCorrVal, reportTime, TIME_STEP);
      ...
      {
        patternCount++;
        Integer patternVal = null;

        if (patternCount >= 5 && patternCount < 10)
          patternVal = new Integer (patternCount + 10);
        else if (patternCount >= 10 && patternCount <= 15)
          patternVal = new Integer (patternCount + 25);
        else
          patternVal = new Integer (rand.nextInt (20));;

        if (patternCount == 25)
          patternCount = 0;

        TimeSeriesValue patternTSV = new TimeSeriesValue
                    ("PatternIdentificationData", patternVal, reportTime, TIME_STEP);
        publishAdd (patternTSV);

      }


      if (this.mixedSignalStarted)
      {
        Integer mixedVal = null;

        if (mixedPeriodCount >= 0 && mixedPeriodCount < 40)
        {
          if (mixedPeriodCount >= mixedPatternOffset &&
              (mixedPeriodCount < (mixedPatternOffset + mixedPatternSize)))
          {
            if (preCursorCount < 3)
              mixedVal = new Integer (140);
            else
              mixedVal = new Integer (175);

            preCursorCount++;
          }
          else
            mixedVal = new Integer (rand.nextInt(125));
        }
        else
          mixedVal = new Integer (200);  // this is the value of the periodic flat-line portion.

        mixedPeriodCount++;

        if (mixedPeriodCount == 45)
        {
          mixedPeriodCount = 0;
          mixedPatternOffset = rand.nextInt(30);
          preCursorCount = 0;
        }

        TimeSeriesValue mixedTSV = new TimeSeriesValue ("MixedSignalData", mixedVal, reportTime, TIME_STEP);
        publishAdd (mixedTSV);

      } // end if (this.mixedSignalStarted)
