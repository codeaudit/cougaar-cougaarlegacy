...
//        df.addText (tsr.toString());
        ...
        else if (tsName.equals ("MixedSignalData"))
          testAlgorithmCombination (ts);
  ...
  private ExactMatchAlgorithm ema = null;
  private AutoCorrelationAlgorithm autoCorrelateCombo = null;
  private PatternPredictionAlgorithm patternPredictCombo = null;

  private void testAlgorithmCombination (TimeSeries ts)
  {
    if (autoCorrelateCombo == null || patternPredictCombo == null)
    {
      String testName = ts.getName();
      autoCorrelateCombo = new AutoCorrelationAlgorithm (testName);
      patternPredictCombo = new PatternPredictionAlgorithm (testName);

      autoCorrelateCombo.initialize (ts.getTimeIncrement(),ts.getStartTime());
      patternPredictCombo.initialize (ts.getTimeIncrement(),ts.getStartTime());

      df.addText ("algorithm combination started ");
    }
    else
    {
      long recentReportTime = ts.getLastStartTime();

//      System.out.println ("Starting prediction updates");
      autoCorrelateCombo.updatePredictions (recentReportTime , ts);
 //     System.out.println ("Done with autoCorrelation prediction updates");
      patternPredictCombo.updatePredictions (recentReportTime , ts);
  //    System.out.println ("Done with pattern prediction updates");
      long getFrom = 0;

      TimeSeries correlatePredictions = autoCorrelateCombo.getPredictions(getFrom);
      TimeSeries patternPredictions = patternPredictCombo.getPredictions(getFrom);

      ForecastDisplayRequest fdr;

//      System.out.println ("/nWe have our predictions, ready to start merging them.");

      if (correlatePredictions != null && patternPredictions != null)
      {
        System.out.println ("We have two Non-Null predictions, let's merge them");
        TimeSeries combinedPredictions = TimeSeriesUtilities.mergeTimeSeries
                ("MixedSignalPredictions", patternPredictions, correlatePredictions);
        fdr = new ForecastDisplayRequest (combinedPredictions, ts, null);
      }
      else if (correlatePredictions != null && patternPredictions == null)
        fdr = new ForecastDisplayRequest (correlatePredictions, ts, null);
      else if (correlatePredictions == null && patternPredictions != null)
        fdr = new ForecastDisplayRequest (patternPredictions, ts, null);
      else
        fdr = new ForecastDisplayRequest (null, ts, null);

 //     System.out.println ("merging attempt finished.");

      fdr.setName ("MixedSignalAnalysis");
      publishAdd (fdr);
    }
